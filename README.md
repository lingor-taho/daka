...
- 员工可多次暂停、恢复；操作使用服务器时间（UTC 存储），按程序时区归属当天。
- 暂停不会退勤，刷新或换到该员工的另一台已授权设备也不会自动恢复。
- 退勤后的净工作时长 = 上下班间隔减去所有暂停区间。暂停中直接退勤，暂停之后的时间不计入。
- 没有退勤时，以最后一次暂停时间截止计算，保留“未退勤”事实；即使之后恢复，恢复后的时间也要等退勤后才计入。没有暂停也没有退勤时，不猜测结束时间。
- 后台月历和 Excel 使用同一套净时长计算；考勤详情可查看每次暂停/恢复时间。管理员修正上下班时间时保留暂停记录，只扣除修正区间内的暂停时间。
- 首次启动新版会自动创建暂停记录表，不改动旧考勤。清空考勤历史时，其暂停记录同步删除。
